PROGRAM wave
  ...
  !Put data in the window on the local processor
  recv_rank = rank
  ...
  ALLOCATE(ranks(1:nproc-1))
  IF (rank == 0) THEN
    DO icycle = 1, nproc - 1
      CALL MPI_Get(ranks(icycle), 1, MPI_INTEGER, icycle, offset, 1, &
  ...
  IF (rank == 0) THEN
    PRINT *,'Data gathered by put is ', ranks
  END IF
