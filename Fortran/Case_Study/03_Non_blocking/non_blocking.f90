...
!        WRITE(*,'(A)', ADVANCE='NO') vals(index) // " "
  ...
  SUBROUTINE bcs(array)
    ...
    CALL MPI_Isend(array, 1, type_l_s, x_min_rank, tag, cart_comm, &
        requests(1), ierr)
    CALL MPI_Isend(array, 1, type_r_s, x_max_rank, tag, cart_comm, &
        requests(2), ierr)
    CALL MPI_Isend(array, 1, type_d_s, y_min_rank, tag, cart_comm, &
        requests(3), ierr)
    CALL MPI_Isend(array, 1, type_u_s, y_max_rank, tag, cart_comm, &
        requests(4), ierr)

    CALL MPI_Irecv(array, 1, type_l_r, x_min_rank, tag, cart_comm, &
        requests(5), ierr)
    CALL MPI_Irecv(array, 1, type_r_r, x_max_rank, tag, cart_comm, &
        requests(6), ierr)
    CALL MPI_Irecv(array, 1, type_d_r, y_min_rank, tag, cart_comm, &
        requests(7), ierr)
    CALL MPI_Irecv(array, 1, type_u_r, y_max_rank, tag, cart_comm, &
        requests(8), ierr)
