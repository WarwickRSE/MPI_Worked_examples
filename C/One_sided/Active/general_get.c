...
int main(int argc, char ** argv)
{
  ...
  if (rank == 0) {
    ...
    for (icycle = 0; icycle< nproc-1; ++icycle){
      ranks[icycle] = icycle+1;
    }
  ...
  if (rank == 0) {
    ...
    for (icycle = 1; icycle<nproc; ++icycle){
      MPI_Get(ranks+(icycle-1), 1, MPI_INT, icycle, offset, 1, MPI_INT, window);
    }
  }
  ...
  if (rank == 0) {
    printf("Data gathered by put is ");
    for (icycle = 0; icycle < nproc-1; ++icycle){
      printf("%3d ", ranks[icycle]);
    }
    printf("\n");
  }
