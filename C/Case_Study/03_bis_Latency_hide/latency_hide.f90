...
PROGRAM serial
  ...
  DO icycle = 1, 500
    ...
    !Because the operating stencil is only one cell on each side, we can touch
    !All of the cells at least one in from the egde before the MPI has finished
    DO iy = 2, ny_local - 1
      DO ix = 2, nx_local - 1
    ...
    !Now to do the egdes you have to wait for the comms to finish
    !Only need to wait for receives to finish at this point
    CALL MPI_Waitall(4, requests(5:8), statuses(:,5:8), ierr)

    !Now actually compute the outer strips
    DO ix = 1, nx_local
      temp_local(ix,1) = 0.25 * (values_local(ix+1,1) + &
          values_local(ix,2) + values_local(ix-1,1) + &
          values_local(ix,0))
      temp_local(ix,ny_local) = 0.25 * (values_local(ix+1,ny_local) + &
          values_local(ix,ny_local + 1) + values_local(ix-1,ny_local) + &
          values_local(ix,ny_local - 1))
    END DO

    DO iy = 1, ny_local
      temp_local(1,iy) = 0.25 * (values_local(2,iy) + &
          values_local(1,iy+1) + values_local(0,iy) + &
          values_local(1,iy-1))
      temp_local(nx_local,iy) = 0.25 * (values_local(nx_local+1,iy) + &
          values_local(nx_local,iy+1) + values_local(nx_local-1,iy) + &
          values_local(nx_local,iy-1))
    END DO

    !Now need to wait for the sends too (they will almost certainly have
    !finished, but better to be sure)

    CALL MPI_Waitall(4, requests(1:4), statuses(:,1:4), ierr)
